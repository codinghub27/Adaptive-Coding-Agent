...
/** The FastAPI backend the dev server proxies API calls to. */
...
react(),
    ...
    server: {
      ...
      strictPort: true,
      // Everything the UI calls is proxied to the FastAPI backend, so the
      // browser only ever sees one origin. That is what lets the httpOnly
      // SameSite=Strict refresh cookie work without any CORS credential
      // handling, and it mirrors production, where FastAPI serves the built
      // bundle from the same origin as the API.
      proxy: Object.fromEntries(
        ['/auth', '/chat', '/conversations', '/profile', '/understand', '/health'].map((path) => [
          path,
